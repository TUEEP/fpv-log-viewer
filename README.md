...
可直接在浏览器上传 EdgeTX 导出的 CSV，进行 2D/3D 轨迹查看、回放，以及基于当前回放帧的点位详情分析。
...
- 点位详情面板（跟随当前回放帧更新，桌面端显示在右侧，小屏显示在视图下方）
- 平滑轨迹曲线显示
- 3D 空间轨迹（可拖动旋转、缩放）
- 高度坐标系切换（`Alt(m)#1` / `Alt(m)#2`）
- 3D Z 轴缩放（高度可视化增强）
- 回放控制（播放/暂停/减速/加速/倍速选择/拖拽时间轴）
- 全屏模式
- 中/英/日语言切换
- 明亮/暗黑主题切换
...
6. 底部使用回放控制进行连续播放与倍速调节；拖动时间轴可查看对应帧的点位详情。
7. 可切换语言、主题、全屏模式。
